...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device=get_device_type(), torch_dtype=torch.bfloat16):
        ...
        self.ocr_image_encoder = None
        self.ocr_loss = None
        self.style_image_encoder = None
        self.style_encoder_type = None
    ...
    def from_pretrained(
        ...
        add_ocr_embedding: bool = False,
        enable_ocr_loss: bool = False,
        ocr_loss_config: Optional[dict] = None,
        add_style_embedding: bool = False,
        style_encoder_type: str = "qwen_vl_embedding",
        ...
        if add_ocr_embedding:
            proj_dim = pipe.vace.vace_blocks[0].dim if pipe.vace is not None else getattr(pipe.dit, "dim", None)
            from ..models.ocr_image_encoder import OCRImageEncoder
            pipe.ocr_image_encoder = OCRImageEncoder(device=pipe.device, proj_dim=proj_dim, dtype=pipe.torch_dtype)
            pipe.ocr_image_encoder.eval()
            pipe.ocr_image_encoder.requires_grad_(False)
            print(f"🚀 init ocr_image_encoder success, proj_dim={proj_dim}")

        if enable_ocr_loss:
            from ..models.ocr_loss import OCRLoss
            if ocr_loss_config is None:
                ocr_weights_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "models", "ocr_weights"))
                ocr_loss_config = {
                    "rec_model_dir": os.path.join(ocr_weights_dir, "ppv3_rec.pth"),
                    "rec_char_dict_path": os.path.join(ocr_weights_dir, "ppocr_keys_v1.txt"),
                    "device": pipe.device,
                    "dtype": pipe.torch_dtype,
                }
            else:
                ocr_loss_config = dict(ocr_loss_config)
                ocr_loss_config.setdefault("device", pipe.device)
                ocr_loss_config.setdefault("dtype", pipe.torch_dtype)
            pipe.ocr_loss = OCRLoss(**ocr_loss_config)
            print("🚀 init ocr_loss success")

        if add_style_embedding:
            proj_dim = pipe.vace.vace_blocks[0].dim if pipe.vace is not None else getattr(pipe.dit, "dim", None)
            from ..models.style_image_encoder import (
                QwenVitStyleEncoder,
                QwenVlmStyleEncoder,
                QwenVlEmbeddingStyleEncoder,
            )
            encoder_map = {
                "qwen_vit": QwenVitStyleEncoder,
                "qwen_vlm": QwenVlmStyleEncoder,
                "qwen_vl_embedding": QwenVlEmbeddingStyleEncoder,
            }
            encoder_key = style_encoder_type.lower()
            if encoder_key not in encoder_map:
                raise ValueError(
                    f"Unsupported style_encoder_type={style_encoder_type}. "
                    f"Available: {sorted(encoder_map.keys())}"
                )
            pipe.style_image_encoder = encoder_map[encoder_key](out_dim=proj_dim, proj_type=proj_type).to(device=pipe.device, dtype=pipe.torch_dtype)
            pipe.style_encoder_type = encoder_key
            pipe.style_image_encoder.eval()
            pipe.style_image_encoder.requires_grad_(False)
            print(f"🚀 init style_image_encoder success, type={encoder_key}, proj_type={proj_type}, proj_dim={proj_dim}")
    ...
    def __call__(
        ...
        vace_style_image: Optional[Union[Image.Image, list[Image.Image], list[list[Image.Image]]]] = None,
        vace_scale: Optional[float] = 1.0,
        vace_preprocess_type: Optional[Literal["remain_all", "remain_first", "remain_none"]] = "remain_all",
        # Animate
        ...
        inputs_shared = {
            ...
            "vace_video": vace_video, "vace_video_mask": vace_video_mask, "vace_reference_image": vace_reference_image, "vace_style_image": vace_style_image, "vace_scale": vace_scale, "vace_preprocess_type": vace_preprocess_type,
            "gt_text": gt_text,
            "src_text": src_text,
...
class WanVideoUnit_VACE(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("vace_video", "vace_video_mask", "vace_reference_image", "vace_style_image", "vace_scale", "vace_preprocess_type", "height", "width", "num_frames", "tiled", "tile_size", "tile_stride", "gt_text", "src_text"),
            output_params=("vace_context", "vace_scale", "ocr_context", "style_context"),
            onload_model_names=("vae", "ocr_image_encoder", "style_image_encoder")
        )

    def _encode_ocr_context(self, pipe: WanVideoPipeline, gt_text):
        if pipe.ocr_image_encoder is None or gt_text is None:
            return None
        if isinstance(gt_text, (list, tuple)):
            gt_text = gt_text[0] if len(gt_text) > 0 else ""
        gt_text = str(gt_text)
        if len(gt_text) == 0:
            return None
        train_ocr_proj = (
            torch.is_grad_enabled()
            and hasattr(pipe.ocr_image_encoder, "proj")
            and pipe.ocr_image_encoder.proj is not None
            and any(param.requires_grad for param in pipe.ocr_image_encoder.proj.parameters())
        )
        if train_ocr_proj:
            ocr_context = pipe.ocr_image_encoder(gt_text)
        else:
            with torch.no_grad():
                ocr_context = pipe.ocr_image_encoder(gt_text)
        if ocr_context.ndim == 2:
            ocr_context = ocr_context.unsqueeze(0)
        return ocr_context.to(dtype=pipe.torch_dtype, device=pipe.device)

    def _normalize_qwen_vlm_gt_text(self, gt_text, batch_size: int):
        if batch_size <= 0:
            return ""
        if gt_text is None:
            return ""
        ...
        if isinstance(gt_text, (list, tuple)):
            if len(gt_text) == 0:
                return ""
            normalized = ["" if item is None else str(item) for item in gt_text]
            if batch_size == 1:
                return normalized[0]
    ...
    def _encode_style_context(self, pipe: WanVideoPipeline, vace_style_image, gt_text):
        if pipe.style_image_encoder is None or vace_style_image is None:
            return None

        if isinstance(vace_style_image, (tuple, list)):
            if len(vace_style_image) == 0:
                return None
            if isinstance(vace_style_image[0], (tuple, list)):
                style_batch = [list(item) for item in vace_style_image]
            else:
                style_batch = [list(vace_style_image)]
        else:
            style_batch = [[vace_style_image]]

        proj_module = getattr(pipe.style_image_encoder, "style_proj_out", None)
        if proj_module is None:
            proj_module = getattr(pipe.style_image_encoder, "proj", None)

        train_style_proj = (
            torch.is_grad_enabled()
            and proj_module is not None
            and any(param.requires_grad for param in proj_module.parameters())
        )

        style_encoder_kwargs = {}
        is_qwen_vlm = (
            getattr(pipe, "style_encoder_type", None) == "qwen_vlm"
            or pipe.style_image_encoder.__class__.__name__ == "QwenVlmStyleEncoder"
        )
        if is_qwen_vlm:
            style_encoder_kwargs["gt_text"] = self._normalize_qwen_vlm_gt_text(gt_text, len(style_batch))

        if train_style_proj:
            style_context = pipe.style_image_encoder(style_batch, **style_encoder_kwargs)
        else:
            with torch.no_grad():
                style_context = pipe.style_image_encoder(style_batch, **style_encoder_kwargs)

        if style_context.ndim == 2:
            style_context = style_context.unsqueeze(0)
        return style_context.to(dtype=pipe.torch_dtype, device=pipe.device)

    def process(
        self,
        pipe: WanVideoPipeline,
        vace_video, vace_video_mask, vace_reference_image, vace_style_image, vace_scale, vace_preprocess_type,
        height, width, num_frames,
        tiled, tile_size, tile_stride,
        gt_text, src_text,
    ):
        if pipe.ocr_image_encoder is not None:
            pipe.load_models_to_device(["ocr_image_encoder"])
        if pipe.style_image_encoder is not None:
            pipe.load_models_to_device(["style_image_encoder"])
        ocr_context = self._encode_ocr_context(pipe, gt_text)
        style_context = self._encode_style_context(pipe, vace_style_image, src_text)

        if vace_video is not None or vace_video_mask is not None or vace_reference_image is not None:
            pipe.load_models_to_device(["vae"])
            has_vace_video_mask = vace_video_mask is not None
            ...
            if vace_preprocess_type is None:
                vace_preprocess_type = "remain_all"
            if vace_preprocess_type not in ("remain_all", "remain_first", "remain_none"):
                raise ValueError(
                    f"Unsupported vace_preprocess_type={vace_preprocess_type}. "
                    "Available: ['remain_all', 'remain_first', 'remain_none']"
                )

            if has_vace_video_mask and vace_preprocess_type != "remain_all":
                gray_value = vace_video.new_tensor(127.0 * 2.0 / 255.0 - 1.0)
                preprocess_mask = (vace_video_mask[:, :1] > 0.5).to(dtype=vace_video.dtype)
                if vace_preprocess_type == "remain_first":
                    if preprocess_mask.shape[2] > 1:
                        preprocess_mask = preprocess_mask.clone()
                        preprocess_mask[:, :, 0] = 0
                    else:
                        preprocess_mask = torch.zeros_like(preprocess_mask)
                vace_video = vace_video * (1 - preprocess_mask) + gray_value * preprocess_mask
            ...
            return {"vace_context": vace_context, "vace_scale": vace_scale, "ocr_context": ocr_context, "style_context": style_context}
        else:
            return {"vace_context": None, "vace_scale": vace_scale, "ocr_context": ocr_context, "style_context": style_context}
...
def model_fn_wan_video(
    ...
    vace_context = None,
    ocr_context: Optional[torch.Tensor] = None,
    ...
    **kwargs,
    ...
    def _append_extra_context(base_context: torch.Tensor, extra_context: Optional[torch.Tensor]):
        if extra_context is None:
            return base_context
        extra_context = extra_context.to(device=base_context.device, dtype=base_context.dtype)
        if extra_context.ndim == 2:
            extra_context = extra_context.unsqueeze(0)
        if extra_context.shape[0] != base_context.shape[0]:
            if extra_context.shape[0] == 1:
                extra_context = extra_context.expand(base_context.shape[0], -1, -1)
            elif base_context.shape[0] % extra_context.shape[0] == 0:
                extra_context = extra_context.repeat(base_context.shape[0] // extra_context.shape[0], 1, 1)
            else:
                extra_context = extra_context[:1].expand(base_context.shape[0], -1, -1)
        if extra_context.shape[-1] != base_context.shape[-1]:
            if extra_context.shape[-1] > base_context.shape[-1]:
                extra_context = extra_context[:, :, :base_context.shape[-1]]
            else:
                dim_pad = base_context.shape[-1] - extra_context.shape[-1]
                extra_context = torch.nn.functional.pad(extra_context, (0, dim_pad), value=0)
        return torch.cat([base_context, extra_context], dim=1)

    context_vace = _append_extra_context(context_vace, ocr_context)
    context_vace = _append_extra_context(context_vace, style_context)
    
    if vace_context is not None:
        vace_hints = vace(
            x, vace_context, context_vace, t_mod, freqs,
            use_gradient_checkpointing=use_gradient_checkpointing,
            use_gradient_checkpointing_offload=use_gradient_checkpointing_offload
        )
