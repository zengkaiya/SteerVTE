...
def get_image_file_list(img_file):
    ...
    if len(imgs_lists) == 0:
        raise Exception("not found any img file in {}".format(img_file))
    imgs_lists = sorted(imgs_lists, key=lambda x: int(os.path.splitext(os.path.basename(x))[0]))
    return imgs_lists
...
class TextRecognizer(object):
    ...
    def resize_norm_img(self, img, max_wh_ratio, norm=True):
        ...
        if norm:
            ...
            resized_image -= 0.5
            resized_image /= 0.5
        padding_im = torch.zeros((imgC, imgH, imgW), dtype=resized_image.dtype, device=img.device)
        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            if show_debug:
                for i in range(len(norm_img_batch)):
                    ...
                    _img = (_img + 0.5)*255
                    _img = _img[:, :, ::-1]
                    file_name = f'tmp/ref_img/{indices[beg_img_no + i]}'
            ...
            if self.is_onnx:
                ...
                # Keep recognizer input dtype/device consistent with model weights.
                try:
                    model_param = next(self.predictor.parameters())
                    norm_img_batch = norm_img_batch.to(device=model_param.device, dtype=model_param.dtype)
    ...
    def pred_img(self, img, show_debug=False):
        """
        img: torch.Tensor, shape (C, H, W), value range 0-255
        """
        assert img is not None

        # normalize to model input range if needed
        if not torch.is_floating_point(img):
            img = img.float()
        img = img / 255.0
        img = img - 0.5 # [0, 1] -> [-0.5, 0.5]
        img /= 0.5 # [-0.5, 0.5] -> [-1, 1]
        ...
        if show_debug:
            _img = img[0].permute(1, 2, 0).detach().cpu().numpy()
            _img = (_img + 0.5) * 255
            _img = _img[:, :, ::-1]
            cv2.imwrite("debug_pred_img.jpg", _img)

        # inference
        if self.is_onnx:
            input_dict = {
                self.predictor.get_inputs()[0].name: img.detach().cpu().numpy()
            }
            outputs = self.predictor.run(None, input_dict)
            preds_ctc = torch.from_numpy(outputs[0])[0]
            preds_ctc_neck = torch.zeros(1)
        else:
            try:
                model_param = next(self.predictor.parameters())
                img = img.to(device=model_param.device, dtype=model_param.dtype)
            except StopIteration:
                pass
            preds = self.predictor(img)
            preds_ctc = preds['ctc'][0]
            preds_ctc_neck = preds['ctc_neck'][0]

        return preds_ctc, preds_ctc_neck
...
def main():
    ...
    args.rec_char_dict_path = str(DEFAULT_OCR_CHAR_DICT_PATH)
    args.rec_batch_num = 5
    args.use_fp16 = False
    text_recognizer = TextRecognizer(args, predictor)
    text_recognizer.predictor.to('cuda')

    image_file_list = ["tmp.png"]
    gt_text = ['Suv']
    print('sample gt text:', gt_text[0])
    ...
    for image_file in image_file_list:
        ...
        img_list.append(torch.from_numpy(img).permute(2, 0, 1).float().to('cuda'))
    try:
        ...
        for i, image_path in enumerate(valid_image_file_list):
            pred_prob = preds_all[i]
            order, _ = text_recognizer.decode(pred_prob)
            pred_text = text_recognizer.get_text(order)

            gt = gt_text[i]
            l = float(loss[i].detach().cpu().numpy())
            ...
            print(f"image_path: {image_path}, pred: {pred_text}, gt: {gt}, loss: {l:.6f}")
    except Exception as E:
        print(traceback.format_exc(), E)

    loss_values = np.array([v["loss"] for v in results.values()])

    print("\n====== Loss Statistics ======")
    print(f"count : {len(loss_values)}")
    print(f"min   : {loss_values.min():.6f}")
    print(f"max   : {loss_values.max():.6f}")
    print(f"mean  : {loss_values.mean():.6f}")
    print(f"median: {np.median(loss_values):.6f}")
    print(f"std   : {loss_values.std():.6f}")
